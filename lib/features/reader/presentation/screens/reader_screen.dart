...
class _ReaderScreenState extends State<ReaderScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    if (mounted) {
      setState(() {
        if (bundle != null) {
          _toc = bundle.textEditionStructures[widget.editionId] ?? 
  ...
  void _handleHorizontalSwipe(DragEndDetails details) {
    ...
    if (details.primaryVelocity! < -300) {
      if (currentIndex >= 0 && currentIndex < flatToc.length - 1) {
        context.pushReplacement('/book/${widget.slug}/${widget.editionId}/${flatToc[currentIndex + 1].id}'); // using id as slug for now or assuming id matches slug
      }
    } 
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    onTap: () {
                      ...
                      if (!isCurrent) {
                        context.pushReplacement('/book/${widget.slug}/${widget.editionId}/${item.id}');
                      }
                    },
                  );
                },
